...
if PC == 8030:
    ...
    train_path = r'E:\LiuYi\dataset\Remote_CL\train_taskB_CL0'  # 对应机器的训练路径
    test_path = r"E:\LiuYi\dataset\Remote_CL\test_taskB"  # 对应机器的验证集路径
    parser.add_argument('--num_classes', type=int, default=20)  # 设定输出维度
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    nw = 10  # num_worker
elif PC == 'taskB28030':
    train_path = r'E:\LiuYi\dataset\Remote_CL\train_taskB_CL1'  # 对应机器的训练路径
    test_path = r"E:\LiuYi\dataset\Remote_CL\test_taskB"  # 对应机器的验证集路径
    parser.add_argument('--num_classes', type=int, default=20)  # 设定输出维度
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    nw = 10  # num_worker
elif PC == 'taskB38030':
    train_path = r'E:\LiuYi\dataset\Remote_CL\train_taskB_CL2'  # 对应机器的训练路径
    test_path = r"E:\LiuYi\dataset\Remote_CL\test_taskB"  # 对应机器的验证集路径
    parser.add_argument('--num_classes', type=int, default=20)  # 设定输出维度
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    nw = 10  # num_worker
...
parser.add_argument('--max_timing', type=int, default=60*60*40)  # 设定输出维度
parser.add_argument('--epochs', type=int, default=10)  # 训练迭代次数
parser.add_argument('--batch_size', type=int, default=128)  # 设定批大小
parser.add_argument('--lr_min', type=float, default=0.00001)  # 设定初始学习率
parser.add_argument('--lr_max', type=float, default=0.001)  # 设定初始学习率
# parser.add_argument('--lrf', type=float, default=0.01)      # 没用
parser.add_argument('--model_name', default='', help='create model name')
parser.add_argument('--transfer_flag', type=bool, default=True)  # 迁移自己的权重
# 预训练权重路径，如果不想载入就设置为空字符
parser.add_argument('--weights', type=str, default='./taskA8030_acc_max0_9859375_epoch13_trial1_2023-08-15-18-40.pth',
                    help='initial weights path')  # taskA best
# parser.add_argument('--weights', type=str, default='./taskB18030_acc_max0_9610909090909091_epoch9_trial7_2023-08-22-10-25.pth',
#                     help='initial weights path')  # taskB1 best
# parser.add_argument('--weights', type=str, default='./taskB28030_acc_max0_9598421541318477_epoch7_trial1_2023-08-22-16-22.pth',
#                     help='initial weights path')  # taskB2 best
# 是否冻结权重
...
def objective(trial):
    ...
    writer = SummaryWriter(str(PC) + '_'+"optuna_with_my_model_runs" + "/" + 'LR' + str(lr) + '_BATCH' + str(args.batch_size) + '_EPOCH' + str(
        args.epochs) + '_CLASS' + str(args.num_classes) + '_FREEZE' + str(args.freeze_layers) + '_' + str(
        timestamp) + '_' + str(curr_time.hour) + '_' + str(curr_time.minute))  # tensorboard
    accu_num = torch.zeros(1).to(device)  # 累计预测正确的样本数 # tensorboard
    sample_num = 0

    for epoch in range(args.epochs):  # 循环训练epoch次
        loss_train_list = []    # 每次新的epoch训练开始前，情况上个epoch记录到的loss 方便记录此次epoch的平均loss
        ...
        if acc_now > acc_max_global:  # 当前epoch验证集正确率创了新高
            ...
            model_accmax_for_test_name = str(PC) + '_' \
                                         ...
                                         + str(timestamp) + '-' + str(curr_time.hour) + '-' + str(
                curr_time.minute) + '.pth'
